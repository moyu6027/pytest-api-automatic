...
RUN apt-get update && apt-get install -y wget default-jdk && cd /opt && \
    (wget -c https://dl.bintray.com/qameta/generic/io/qameta/allure/allure/2.8.40/allure-2.8.40.tgz -O - | tar -xz && chmod +x allure-2.8.40/bin/allure)
ENV PATH="${PATH}:/opt/allure-2.8.40/bin"
RUN allure --version
